...
def login(s, user, password):
    response = send_request(
        s, LEARN_URL + '/auth-saml/saml/login',
        params={'apId': '_175_1'}
    )
    soup = BeautifulSoup(response.text, 'lxml')

    response = send_request(
        s, soup.find('form').get('action'), method='POST',
    ...
    for key in ['submit', 'ref', 'service']:
        loginfo[key] = login_form.findChild('input', attrs={'name': key}) \
                                 .get('value')
    response = send_request(
        s, EASE_URL + login_form.get('action'),
        method='POST', data=loginfo
    )
    soup = BeautifulSoup(response.text, 'lxml')
...
def send_request(s, url, method='GET', raise_on_error=True, **kwargs):
    try:
        ...
        if raise_on_error:
            raise
        logger.error(
            '%s when trying to access %s - skipping...', e, url
        )
        return None
...
def scrape():
    ...
    logger.info(
        'Successfully logged in to Learn in %ss',
        time.time() - start
    )
    ...
    logger.info(
        ...
        len(COURSE_INFOS), time.time() - start
    )
    ...
    logger.info(
        ...
        len(CONTENT_INFOS), len(FOLDER_INFOS),
        len(COURSE_INFOS), time.time() - start
    )
    ...
    for idx, (path, link) in enumerate(CONTENT_INFOS, 1):
        ...
        while not response:
            response = send_request(
                s, LEARN_URL + link.get('href').replace(LEARN_URL, ''),
                raise_on_error=False, timeout=5
            )
